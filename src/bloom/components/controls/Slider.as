...
//		_button.drawDirectly ();
	...
	protected function onBgMouseDown (e:MouseEvent):void {
		if (_type == ComponentConstants.HORIZONTALLY) {
			_value = ((mouseX - (_height >> 1)) / (_width - _height)) * (_max - _min) + _min;
		} else {
			_value = (_height - (_width >> 1) - mouseY) / (_height - _width) * (_max - _min) + _min;
		}
